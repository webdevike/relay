...
function harness(permission: PermissionOutcome = "granted"): Harness {
  ...
  let inFlight: { promise: Promise<unknown>; settle: (outcome?: { code: string }) => void } | null =
    null;
...
describe("dictationMachine", () => {
  it("pressStop with submit carries through to the delivery, and does not persist into the next dictation", async () => {
    ...
    expect(h.sends).toEqual([
      { text: "ship it", submit: true, skill: null, images: [], pasted: null, launch: false },
    ]);
  ...
  it("any release ends the dictation, however short the press", async () => {
    ...
    expect(h.sends).toEqual([
      { text: "quick one", submit: false, skill: null, images: [], pasted: null, launch: false },
    ]);
  });

  /** A machine whose permission check only resolves when the test says so. */
  function slowPermission(): {
    actor: Actor<typeof dictationMachine>;
    clock: SimulatedClock;
    grant: () => void;
  } {
  ...
  it("a lost release is recovered by the next press", async () => {
    ...
    expect(h.sends).toEqual([
      { text: "still here", submit: false, skill: null, images: [], pasted: null, launch: false },
    ]);
  });
  ...
  it("launch finishes the dictation and delivers it as a session start; the orb flies without a release", async () => {
    const h = harness();
    ...
    expect(h.phase()).toBe("finishing");
    expect(h.orb()).toBe("flying");
    h.actor.send({ type: "final", text: "plan the dashboard rework" });
    expect(h.phase()).toBe("sending");
    expect(h.recognizerAlive()).toBe(false);
    expect(h.sends).toEqual([
      { text: "plan the dashboard rework", submit: false, skill: "/skill:deploy", images: [], pasted: null, launch: true },
    ]);
    await h.settleSend();
    h.clock.increment(620);
    expect(h.orb()).toBe("gone");
  ...
  it("launch with nothing heard still starts a session, empty", async () => {
    const h = harness();
    await pressAndListen(h);
    h.actor.send({ type: "launch" });
    h.actor.send({ type: "recognizerEnd" });
    expect(h.phase()).toBe("sending");
    expect(h.sends).toEqual([{ text: "", submit: false, skill: null, images: [], pasted: null, launch: true }]);
  });
  ...
  it("happy path: press, partials, release, final, send, sent, back to idle", async () => {
    ...
    expect(h.actor.getSnapshot().context.transcript).toBe("hello world.");
    expect(h.sends).toEqual([
      { text: "hello world.", submit: false, skill: null, images: [], pasted: null, launch: false },
    ]);
  ...
  it("release with no final within 1.5s uses the last partial", async () => {
    ...
    expect(h.phase()).toBe("sending");
    expect(h.sends).toEqual([
      { text: "take this down", submit: false, skill: null, images: [], pasted: null, launch: false },
    ]);
  });
  ...
  it("the recognizer ending during finishing finalizes with the last partial immediately", async () => {
    ...
    expect(h.phase()).toBe("sending");
    expect(h.sends).toEqual([
      { text: "done talking", submit: false, skill: null, images: [], pasted: null, launch: false },
    ]);
  });
  ...
  describe("skill wheel", () => {
    ...
    it("arming while idle points the next dictation at the skill; arming null clears it", async () => {
      ...
      expect(h.sends).toEqual([
        { text: "ship relay", submit: false, skill: "/goal set", images: [], pasted: null, launch: false },
      ]);
      await h.settleSend();
      h.clock.increment(900);
  ...
  describe("attachments", () => {
    const shot = (n: number): PendingImage => ({
      mimeType: "image/jpeg",
      data: `img${String(n)}`,
      width: 10,
      height: 20,
    ...
    it("sendAttachments sends the images on their own as a submitted, text-free turn", async () => {
      ...
      expect(h.phase()).toBe("sending");
      expect(h.sends).toEqual([
        { text: "", submit: true, skill: null, images: [shot(1)], pasted: null, launch: false },
      ]);
      await h.settleSend();
      expect(h.phase()).toBe("sent");
      expect(h.actor.getSnapshot().context.images).toEqual([]);
    ...
    it("pasted text waits like an image: it goes with the next dictation, or alone, and a null paste drops it", async () => {
      ...
      expect(h.sends).toEqual([
        { text: "explain", submit: false, skill: null, images: [], pasted: "stack trace", launch: false },
      ]);
      await h.settleSend();
      h.clock.increment(900);
    ...
    describe("typed", () => {
      it("sends the trimmed text as a submitted turn with whatever is armed or attached; blank text is ignored", async () => {
        const h = harness();
        h.actor.send({ type: "typed", text: "   " });
        expect(h.phase()).toBe("idle");
        h.actor.send({ type: "arm", skill: "/handoff" });
        h.actor.send({ type: "attach", image: shot(1) });
        h.actor.send({ type: "typed", text: "  ship it  " });
        expect(h.phase()).toBe("sending");
        expect(h.sends).toEqual([
          { text: "ship it", submit: true, skill: "/handoff", images: [shot(1)], pasted: null, launch: false },
        ]);
        await h.settleSend();
        expect(h.phase()).toBe("sent");
        expect(h.actor.getSnapshot().context.images).toEqual([]);
      ...
      it("is dropped while a send is in flight", () => {
        const h = harness();
        h.actor.send({ type: "typed", text: "one" });
        h.actor.send({ type: "typed", text: "two" });
